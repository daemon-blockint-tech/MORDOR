...
def _run_pipeline(
    ...
    try:
        ...
        state = orchestrator._build_initial_state(binary_path, tier)
        state["case_dir"] = str(case_manager.cases_dir / case_id)
        os.makedirs(state["case_dir"], exist_ok=True)
...
async def analyze_path(
    ...
    case_id, _ = case_manager.create_case(str(path.resolve()), tier)
